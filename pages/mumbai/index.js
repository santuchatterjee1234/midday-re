...
import MumbaiComponets from "../../src/components/categorycomponent/index";
import axios from 'axios';
import { config } from '../../next.config.js';
const Category = ({initialCategoryData, categoryName}) => {
  return (
    <div className={`{styles.container} mb-3`}>
      <MumbaiComponets categoryData={initialCategoryData} categoryName={categoryName} videoTagName="Mumbai Meri Jaan" upperBannerTagName="" />
    </div>
  );
}
...
export async function getServerSideProps(context) {
  try { 
    ...
    formatdata.append('category_name', 'mumbai');
